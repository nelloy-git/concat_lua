...
local Player = require('baseClasses.Player')
---@type Settings
...
function DummyAbilityEvent.init()
    ...
    local any_player_press_lshift = Player.getKeyTrigger(OSKEY_LSHIFT)
    any_player_press_lshift:addAction(runFuncInDebug, shiftPressed)

    local any_player_press_rshift = Player.getKeyTrigger(OSKEY_RSHIFT)
    any_player_press_rshift:addAction(runFuncInDebug, shiftPressed)

    local any_player_press_escape = Player.getKeyTrigger(OSKEY_ESCAPE)
    any_player_press_escape :addAction(runFuncInDebug, escapePressed)

    local any_player_press_mouse = Player.getTrigger(EVENT_PLAYER_MOUSE_DOWN)
    any_player_press_mouse:addAction(runFuncInDebug, mousePressed)

    local any_player_mouse_move = Player.getTrigger(EVENT_PLAYER_MOUSE_MOVE)
